...
const highScore = movies2.filter((movie) => 
    movie.score >= 90);
...
// BEFORE
const highScoreMovies = highScore.map(movie => {
    return `${movie.title} ${movie.years} is rated ${movie.score}`;
});


// AFTER Destructuring
const highScoreMovies = highScore.map(({title, years,score}) => {
    return `${title} ${years} is rated ${score}`;
});
